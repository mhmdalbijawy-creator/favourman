    ...
    return (
        <div className={`bg-rich-black min-h-screen pt-32 pb-24 overflow-hidden ${isAr ? 'font-ar' : 'font-en'}`}>
            ...
            <section className="container mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-8">
                {(() => {
                    const sectorItems = t('home.sectors.items', { returnObjects: true }) || [];
                    const items = [
                    ...
                    if (sectorItems.length > 9) {
                        items.push(
                            { title: sectorItems[8].title, desc: sectorItems[8].desc }, // Food Security
                            { title: sectorItems[9].title, desc: sectorItems[9].desc }, // Pharma
                            { title: sectorItems[4].title, desc: sectorItems[4].desc }  // Iron & Steel
                        );
                    }
                    ...
                    return items.map((item, idx) => (
                        <motion.div
                            ...
                            className="p-10 bg-white/5 border border-white/5 rounded-xl hover:border-gold/30 transition-all duration-500 group"
                        >
                    ...
                    <AnimatePresence mode="wait">
                        {isSubmitted ? (
                            ...
                            >
                                ...
                                <h3 className="text-2xl font-bold text-gold mb-4 uppercase tracking-[0.2em]">{t('imports_exports.success_msg')}</h3>
                            </motion.div>
                        ) : (
                            <form
                                onSubmit={async (e) => {
                            ...
                            >
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                                    <div className="group relative">
                                        <label className="text-[10px] uppercase tracking-[0.3em] font-bold text-gold/50 block mb-4 group-focus-within:text-gold transition-colors">
                                            {t('imports_exports.goods_type')}
                                        </label>
                                        <input required name="goods_type" type="text" className="w-full bg-transparent border-b border-white/10 py-4 text-white focus:outline-none focus:border-gold transition-colors font-light" />
                                    </div>
                                    <div className="group relative">
                                        <label className="text-[10px] uppercase tracking-[0.3em] font-bold text-gold/50 block mb-4 group-focus-within:text-gold transition-colors">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                                    <div className="group relative">
                                        <label className="text-[10px] uppercase tracking-[0.3em] font-bold text-gold/50 block mb-4 group-focus-within:text-gold transition-colors">
                                            {t('imports_exports.origin')}
                                        </label>
                                        <input required name="origin" type="text" className="w-full bg-transparent border-b border-white/10 py-4 text-white focus:outline-none focus:border-gold transition-colors font-light" />
                                    </div>
                                    <div className="group relative">
                                        <label className="text-[10px] uppercase tracking-[0.3em] font-bold text-gold/50 block mb-4 group-focus-within:text-gold transition-colors">
                                            {t('imports_exports.destination')}
                                        </label>
                                        <input required name="destination" type="text" className="w-full bg-transparent border-b border-white/10 py-4 text-white focus:outline-none focus:border-gold transition-colors font-light" />
                                    </div>
                                </div>
                                ...
                                >
                                    <span className="relative z-10 group-hover:text-gold transition-colors duration-700">
                                        {isSubmitting ? t('common.submitting') : t('hero.initiate_protocol')}
                                    </span>
